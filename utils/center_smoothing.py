# Code from https://github.com/aounon/center-smoothing
...
class CenterSmooth(object):
    def __init__(
        self,
        base_function: nn.Module,
        dist_fn,
        sigma: float,
        n_pred: int = 10**4,
        n_cert: int = 10**6,
    ...
    def certify(self, input: torch.tensor, eps_in: float, batch_size: int = 1000):
        with torch.no_grad():
            ...
            quantile = norm.cdf(norm.ppf(min_prob) + (eps_in / self.sigma)) + sqrt(
                log(1 / self.alpha_2) / (2 * self.n_cert)
            )
