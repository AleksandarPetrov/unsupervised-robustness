...
##########################################################################
### This part is based on https://github.com/Separius/SimCLRv2-Pytorch ###
...
def simCLR_get_model(weigths_path):
    model, _ = get_resnet(*name_to_params(weigths_path))
    model.load_state_dict(torch.load(weigths_path)["resnet"])

    return model


simCLR_preprocessing_transform = torchvision.transforms.Compose(
    [
...
simCLR_inverse_transform = torchvision.transforms.Compose(
    [
...
simCLR_training_augmentation = torchvision.transforms.Compose(
    [
